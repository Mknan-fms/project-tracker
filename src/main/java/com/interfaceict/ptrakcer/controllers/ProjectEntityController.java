...
public class ProjectEntityController
{
    ...
    @PutMapping("/{id}")
    public ProjectEntity updateProject(@RequestBody NewProject project, @PathVariable("id") Long id) { return m_Service.update(project, id); }

    @DeleteMapping("/{id}")
    public String deleteProject(@PathVariable("id") Long id) { m_Service.delete(id); return "Deleted Successfully"; }

    /*
        I used json body to update the project status,
        because it is better to keep hidden and not visible in query.
    */
    @PutMapping("/update_project_status/{id}")
    public boolean updateProjectStatus(@PathVariable("id") Long id, @Valid @RequestBody UpdateProjectStatus status) { return m_Service.updateStatus(status.getStatus(), id); }
}
