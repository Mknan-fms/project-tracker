...
public class ProjectDependencyController {
	...
	public List<ProjectDependency> getAllDeps(@PathVariable("project_id") Long projectID) {
		return new ArrayList<>(m_Service.getAllByProjectID(projectID));
	}

	@PostMapping("/")
	public ProjectDependency newDep(@Valid @RequestBody NewDependency dependency) {
		return m_Service.save(dependency);
	}

	@PutMapping("/{dep_id}")
	public ProjectDependency updateDep(@RequestBody NewDependency dependency, @PathVariable("dep_id") Long depID) {
		return m_Service.update(dependency, depID);
	}
